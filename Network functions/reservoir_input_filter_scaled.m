function [ neuron_input, z_all ] = reservoir_input_filter_scaled(SpikeTrainStruct, Ein, N, pole )
disp('Input scaled')
dat = pole;
%% Create the neuron input by multiplying with the input weights matrix
N_th = size(SpikeTrainStruct{1}.ConvTrace, 1);
time = length(SpikeTrainStruct{1}.ConvTrace{1});
ConvTrace = zeros(N_th, time);

for i = 1:N_th
    ConvTrace(i, :) = SpikeTrainStruct{1}.ConvTrace{i};
end

% scale the ConvTrace input
ConvTrace_scaled = zeros(N_th, time);
CT_mean = mean(ConvTrace, 'all');
CT_std = std(ConvTrace, 0, 'all');

for i = 1:N_th
    ConvTrace_signal = ConvTrace(i,:);
    ConvTrace_scaled(i, :) = (ConvTrace_signal - CT_mean)/CT_std;
end

neuron_input = Ein*ConvTrace_scaled;
%% Pulse parameters
